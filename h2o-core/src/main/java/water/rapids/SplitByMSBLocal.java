...
class SplitByMSBLocal extends MRTask<SplitByMSBLocal> {
  ...
  SplitByMSBLocal(boolean isLeft, long base[], int shift, int keySize, int batchSize, int bytesUsed[], int[] col, Key linkTwoMRTask, int[][] id_maps, boolean[] isNotDouble, double[] baseD) {
    _isLeft = isLeft;
    ...
    _shift = shift;
    _batchSize=batchSize; _bytesUsed=bytesUsed; _col=col; _base=base; _isNotDouble=isNotDouble; _baseD=baseD;
    _keySize = keySize;
    ...
    for (int r=0; r<chk[0]._len; r++) {    // tight, branch free and cache efficient (surprisingly)
      ...
      if (!chk[0].isNA(r)) {
        ...
        // able to use as raw, but when we can maybe can do in bulk
          ...
          thisx = _isNotDouble[0]?(chk[0].at8(r) - _base[0] + 1):Double.doubleToRawLongBits(chk[0].atd(r)-_baseD[0]+1);
        }
      ...
      for (int c=1; c<chk.length; c++) {  // TO DO: left align subsequent
        ...
        else thisx = _isNotDouble[c]?thisx - _base[c] + 1:Double.doubleToRawLongBits(chk[c].atd(r)-_baseD[c]+1);
